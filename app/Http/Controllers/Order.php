...
{  
    ...
    public function checkout(Request $request)
    {
        ...
        $total = 0;
        $data = DB::table('tb1_keranjang')->where('id_user', Session::get('id_user'))->get();
        foreach ($data as $krj) {
            $barang = DB::table('tb1_barang')->where('id', $krj->id_barang)->get();
    ...
    public function confirm_save(Request $request)
    {
        ...
        if ($file->move($tujuan_upload,$nama_file)) {
            DB::table('tb1_konfirmasi')->insert([
                'id_user' => Session::get('id_user'),
